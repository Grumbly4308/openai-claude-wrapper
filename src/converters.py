...
class MessagePreparer:
    ...
    def __init__(self, file_store: FileStore, workspace_root: Path, http_client: Optional[httpx.AsyncClient] = None):
    ...
    async def prepare_messages(
        ...
        - ``system`` messages become a header.
        - Earlier user/assistant turns are included verbatim so the model
          sees full conversation context (OpenAI-style stateless requests).
...
def derive_session_id(messages: list[ChatMessage], explicit: Optional[str], user: Optional[str]) -> str:
    ...
    Priority: explicit request field > OpenAI ``user`` field-prefixed hash
    of the conversation-so-far > fresh UUID.
    """
    if explicit:
        return _clean_session_id(explicit)

    # Hash everything except the very last user turn so that subsequent
    # turns with the same preceding history share a session id and Claude
    # Code can reuse context.
    if len(messages) >= 2:
        to_hash = messages[:-1]
        h = hashlib.sha256()
        for m in to_hash:
